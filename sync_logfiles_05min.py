...
for project, project_info in info_dict.items():
    ...
    ## For 3 DAS
    #query_string = """SELECT * FROM log_files WHERE (das_id = %(das_id)s OR das_id = %(das_id2)s OR das_id = %(das_id3)s) AND date_added >= %(from_date)s AND date_added < %(to_date)s AND log_files.id in (SELECT logfile_import_status.logfile_id from logfile_import_status where logfile_import_status.importSkyspark_tenantBilling=TRUE)"""
